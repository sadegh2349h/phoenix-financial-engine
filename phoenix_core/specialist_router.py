...
SPECIALISTS: tuple[Specialist, ...] = (
    Specialist("psyche", "Phoenix Psyche", "consumer psychology and behavioral economics", ("ideal_customer", "hidden_objection", "purchase_fear", "identity_shift", "messaging"), ("psychographic_hypotheses", "core_desire", "hidden_objection", "psychological_trigger", "messaging_angle")),
    Specialist("ops", "Phoenix Ops", "operations and systems thinking", ("repetitive_task", "bottleneck", "workflow", "automation", "delegation", "sop"), ("process_map", "bottleneck", "automation", "delegation", "sop", "success_metrics")),
    Specialist("brand", "Phoenix Brand", "positioning and premium brand strategy", ("competitors", "positioning", "misconception", "unfair_advantage", "uvp", "brand_voice"), ("positioning_statement", "brand_voice", "visual_identity_direction", "content_pillars")),
    Specialist("closer", "Phoenix Closer", "high-ticket sales and negotiation", ("objection", "price_resistance", "closing", "negotiation", "value_alignment", "pricing"), ("objection_scripts", "value_reframe", "low_pressure_next_step", "tiered_pricing")),
    Specialist("growth", "Phoenix Growth", "growth hacking and viral marketing", ("growth", "acquisition", "viral", "referral", "engagement", "algorithm", "community", "challenge"), ("growth_experiments", "acquisition_loop", "referral_loop", "viral_mechanics", "kpi")),
    Specialist("edu", "Phoenix Edu", "instructional design and e-learning", ("course", "workshop", "masterclass", "learning", "training", "transformation", "curriculum"), ("learning_modules", "quick_win", "exercises", "success_metrics", "accountability")),
    Specialist("tribe", "Phoenix Tribe", "community architecture and engagement", ("community", "tribe", "identity", "ritual", "engagement", "retention", "ambassador"), ("onboarding_ritual", "weekly_loop", "recognition", "exclusive_benefits", "contributor_loop")),
    Specialist("price", "Phoenix Price", "value-based pricing and behavioral economics", ("pricing", "price", "value", "roi", "premium", "subscription", "anchoring", "decoy", "investment"), ("tiered_pricing", "value_framing", "pricing_psychology", "roi_metrics", "objection_reframe")),
    Specialist("viral", "Phoenix Viral", "short-form content and algorithmic retention", ("reels", "viral", "retention", "shares", "saves", "hook", "pattern_interrupt", "controversial", "content"), ("reel_script", "hook", "retention_mechanics", "shareable_identity", "editing_direction")),
...
def specialist_registry() -> dict[str, dict[str, Any]]:
    return {s.key: {"name": s.name, "domain": s.domain, "primary_signals": s.primary_signals, "deliverables": s.deliverables} for s in SPECIALISTS}


def route_specialists(*, problem: str, analysis: dict[str, Any] | None = None, max_specialists: int = 2) -> list[dict[str, Any]]:
    ...
    return [{"specialist": s.name, "key": s.key, "score": score, "matched_signals": tuple(matches), "reason": f"matched {len(matches)} evidence signal(s)", "human_approval_required": True} for score, s, matches in ranked[:max_specialists]]


def enrich_client_package(package: dict[str, Any], *, problem: str, max_specialists: int = 2) -> dict[str, Any]:
    enriched = dict(package)
    enriched["specialist_routing"] = route_specialists(problem=problem, analysis=package.get("analysis", {}), max_specialists=max_specialists)
