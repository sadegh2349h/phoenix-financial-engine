...
from phoenix_core.specialist_agents import default_agent_names, SpecialistAgentTeam, SpecialistProfile
...
def test_specialists_can_be_selected_by_domain():
    team = SpecialistAgentTeam()
    team.register_specialist(SpecialistProfile("growth", "business", 20), lambda _: {"confidence": 0.8})
    team.register_specialist(SpecialistProfile("risk", "finance", 10), lambda _: {"confidence": 0.9})
    results = team.run_by_domain({"objective": "x"}, {"finance"})
    assert [r.agent for r in results] == ["risk"]
